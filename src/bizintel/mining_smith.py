...
def plot_top_customers(df_sales: pd.DataFrame, df_customers: pd.DataFrame) -> None:
    """Plot the top 10 customers by total sales."""

    LOG.info("Plotting top customers by sales")

    df = df_sales.copy()

    df["SaleAmount"] = pd.to_numeric(df["SaleAmount"], errors="coerce")

    merged = df.merge(df_customers, on="CustomerID", how="left")

    top_customers = (
        merged.groupby("Name")["SaleAmount"].sum().sort_values(ascending=False).head(10)
    )

    plt.figure(figsize=(10, 6))
    top_customers.plot(kind="bar", color="darkgreen")

    plt.title("Top 10 Customers by Total Sales (CLOSE chart to continue)")
    plt.xlabel("Customer")
    plt.ylabel("Total Sales ($)")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()

    LOG.info("Top customers chart created")
...
def main() -> None:
    ...
    plot_top_customers(df_sales, df_customers)
